...
w_dict = {w.name(): w for w in w_list}

# watermarking methods to test
w_methods = [
     w_dict["ssl watermarking"],
     w_dict["blind watermarking"],
     w_dict["trustmark watermarking"],
     w_dict["dwtDct watermarking"],
     w_dict["dwtDctSvd watermarking"],
     w_dict["rivaGan watermarking"],
     w_dict["stegastamp watermarking"],
     w_dict["arwgan watermarking"],
    [w_dict["ssl watermarking"], w_dict["rivaGan watermarking"]],
    [w_dict["ssl watermarking"], w_dict["arwgan watermarking"]],
    [w_dict["blind watermarking"], w_dict["rivaGan watermarking"]],
    [w_dict["trustmark watermarking"], w_dict["ssl watermarking"]],
    [w_dict["rivaGan watermarking"], w_dict["ssl watermarking"]],
    [w_dict["rivaGan watermarking"], w_dict["blind watermarking"]],
    [w_dict["rivaGan watermarking"], w_dict["stegastamp watermarking"]],
    [w_dict["stegastamp watermarking"], w_dict["ssl watermarking"]],
    [w_dict["stegastamp watermarking"], w_dict["trustmark watermarking"]],
    [w_dict["stegastamp watermarking"], w_dict["rivaGan watermarking"]],
    [w_dict["arwgan watermarking"], w_dict["ssl watermarking"]],
    ]

# attacks to evaluate
shape_attacks = [] \
    + [{'attack': 'none'}] \
    + [{'attack': 'aspect ratio', 'ratio': r} for r in [0.75, 0.85, 1.25]] \
    + [{'attack': 'resize', 'scale': s} for s in [0.5, 0.75, 1.25]] \
    + [{'attack': 'generic crop', 'x1': x1, 'y1': y1, 'x2': x2, 'y2': y2} for x1 in [0.25] for x2 in [1] for y1 in [0.3] for y2 in [0.85, 1]] \
    + [{'attack': 'center crop', 'scale': s} for s in [0.5, 0.8, 1.5]]

rotation_attacks = [] \
    + [{'attack': 'none'}] \
    + [{'attack': 'rotation', 'angle': r, 'fill': [255, 255, 255]} for r in [2, 15, 30]]

overlay_attacks = [] \
    + [{'attack': 'none'}] \
    + [{'attack': 'overlay emoji', 'emoji_path': 'data/wm_logo.png', 'y_pos': y, 'x_pos': x} for y in [0.1, 0.35, 0.65] for x in [0.1, 0.6]]
...
photometric_attacks = [] \
    ...
    + [{'attack': 'jpeg', 'quality': q} for q in [20, 50, 75]] \
    + [{'attack': 'contrast', 'contrast_factor': c} for c in [0.5, 1.5, 2.]] \
    + [{'attack': 'brightness', 'brightness_factor': b} for b in [0.5, 1.5, 2.]] \
    + [{'attack': 'hue', 'hue_factor': h} for h in [-0.25, 0.25, 0.5]]

shape_rotation_attacks = shape_attacks + rotation_attacks

attacks = []
for i0 in range(len(shape_attacks)):
    for i1 in range(len(rotation_attacks)):
        if (shape_attacks[i0]['attack'] == 'none') + \
            (rotation_attacks[i1]['attack'] == 'none') > 0: continue                
        attacks.append([shape_attacks[i0], rotation_attacks[i1]])

for i0 in range(len(shape_rotation_attacks)):
    for i1 in range(len(overlay_attacks)):
        for i2 in range(len(photometric_attacks)):
            if (shape_rotation_attacks[i0]['attack'] == 'none') + \
                (overlay_attacks[i1]['attack'] == 'none') + \
